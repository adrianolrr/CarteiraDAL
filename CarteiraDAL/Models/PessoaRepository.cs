...
namespace CarteiraDAL.Models
{
    public class PessoaRepository : AbstractRepository<Pessoa, int>
    {
        ...
        public override void Delete(Pessoa entity)
        {
            ...
            {
                string sql = "DELETE Pessoas Where PessoaId=@PessoaId";
        ...
        {
            string sql = "Select PessoaId, Nome, Email, Cidade, Endereco FROM Pessoas ORDER BY Nome";
            using (var conn = new SqlConnection(StringConnection))
            {
        ...
        public override Pessoa GetById(int id)
        {
            using (var conn = new SqlConnection(StringConnection))
            {
                string sql = "Select PessoaId, Nome, Email, Cidade, Endereco FROM Pessoas WHERE PessoaId=@PessoaId";
                SqlCommand cmd = new SqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("@PessoaId", id);
                Pessoa p = null;
                try
        ...
        public override void Save(Pessoa entity)
        {
            using (var conn = new SqlConnection(StringConnection))
            {
                string sql = "INSERT INTO Pessoas (Nome, Email, Cidade, Endereco) VALUES (@Nome, @Email, @Cidade, @Endereco)";
                SqlCommand cmd = new SqlCommand(sql, conn);
        ...
        public override void Update(Pessoa entity)
        {
            using (var conn = new SqlConnection(StringConnection))
            {
                string sql = "UPDATE Pessoas SET Nome=@Nome, Email=@Email, Cidade=@Cidade, Endereco=@Endereco Where PessoaId=@PessoaId";
                SqlCommand cmd = new SqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("@PessoaId", entity.PessoaId);
                cmd.Parameters.AddWithValue("@Nome", entity.Nome);
                cmd.Parameters.AddWithValue("@Email", entity.Email);
